...
// This is inverse of Initial Permutation
int FP[64] = {
...
int main()
{
    ...
    //take input of cipher text , key and round
    cout<<"Enter 16 (64 bits) characters cipher text (in hexadecimal):" ;
    cin>>cipher_text;
    ...
    {
        cout << "Cipher text must be 16 hex characters\n";
        return 0;
    }
    ...
    cipher_text = hexToBin(cipher_text);
    key = hexToBin(key);
    ...
    //========================Initial Cipher Split===========================

    // Split cipher into left and right part
    string left = cipher_text.substr(0, 32);
    string right = cipher_text.substr(32, 32);
    ...
    {
        string current_left = left;
        string current_right = right;

        // Reverse DES round formula:
        // Ri-1 = Li
        // Li-1 = Ri XOR f(Li, Ki)
        right = current_left;
        left = xorString(current_right, fFunction(current_left, roundKey[i]));
    }

    cout << "\nAfter Decryption Round:\n";
    cout << "L0: " << left << "\n";
    cout << "R0: " << right << "\n";

    // L0 + R0 is after initial permutation.
    // Apply FP to get original plain text.
    string binary_plain = permute(left + right, FP, 64);
    string hex_plain = binToHex(binary_plain);

    cout << "\nBinary Plain Text:\n";
    cout << binary_plain << "\n";

    cout << "\nHexadecimal Plain Text:\n";
    cout << hex_plain << "\n";
